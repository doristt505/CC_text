...
**五步流程：**
1. **语病检查** - 逐句检查语病、句子通顺度、用词准确性（不动情节和结构）。标注分「硬伤」和「可能是选择」两类，后者不给替代方案，只提示
2. **结构分析** - 先判断文体（常规叙事 / 寓言等非常规），再选对应的创作要素评价体系；参考指定作家的**方法**而非外观
3. **分支创作** - 基于结构分析建议，生成 3 个不同结局方向的完整改写版本
4. **选择定稿** - 在"原文"和 3 个分支版本之间选择一个作为终稿
5. **读者反馈** - 模拟 5 位固定读者视角（普通追更读者/文学评论者/同行写手/挑剔读者/目标受众代表）阅读终稿，并给出区分「真问题 / 单一偏好 / 对立取舍」的汇总
...
这套流程真实失败过一次，值得记录：一篇 2000 字的哲学寓言走完五步，模型列了十条修改，作者全部接受，**每一条单独看都成立，合起来把一篇有棱角的寓言磨成了规范的习作**，作者最后退回了原稿。

由此提炼出的几条机制已经写进 `js/api-client.js` 的 system prompt，改动提示词时不要削弱它们：

**1. 第①步分两类，「可能是选择」不给替代方案**
给了建议作者就容易顺手接受；只指出现象，作者才会真正判断这是不是自己的本意。数据上通过 `annotation.category`（`hard` / `choice`）区分，`choice` 的 `suggestion` 必须为空，`applyAnnotations()` 和 `acceptAll()` 都会跳过它们。

**2. 显式允许输出 0 条**
"列出问题"这个动作本身有偏向——一旦开始列清单就会去找条目，十条看起来比三条认真。这是第①步最主要的失败来源。

**3. 搭配错位可能是题眼**
真实案例：原文"一个不会先他们死去的答案"被判为搭配不当改成了"依靠"，但那个错位正是全篇核心。遇到"这个词好像不该这么用"要先假设它是故意的。

**4. 反模式「回指式补充」**
模型在"不能说破主题"的约束下，会退化成把前文出现过的意象重新拿出来做对照（"它的眼睛不像喉咙上的银纹会亮会暗"）。这不是含蓄，是更啰嗦的直说，而且故事没往前走。补充内容必须用**新材料**。这条写在第②③步的 prompt 里。

**5. 第②步先判断文体**
"人物弧光/冲突张力"是常规短篇的标准，用在寓言上会把它改造成合格的短篇小说。非常规文本改用「处境的自洽性 / 意象的贯穿 / 留白的位置」。返回的 JSON 里有 `genre` 字段记录判断结果。

**6. 第⑤步汇总不向通俗性妥协**
追更读者和目标受众代表反映的是**阅读成本**，不是质量问题。返回的 `synthesis` 分三类（`shared_problems` / `single_view_preferences` / `conflicts`），刻意不把"更好读"当成默认正确的一方。

四个分发版本（本仓库的网页版、`PROMPT.md`、`portable-skill/AGENTS.md`、`.claude/skills/novel-polish/`）的流程内容应保持一致，改了一处记得同步其余三处。
